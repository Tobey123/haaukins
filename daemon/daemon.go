...
func New(conf *Config) (*daemon, error) {
	...
	ef, err := store.NewChallengeFile(conf.ExercisesFile)
	if err != nil {
...
func (d *daemon) CreateUnfinishedEvents() error {
	...
	for _, eventFile := range eventFiles {
		event, err := d.ehost.CreateEventFromEventFile(context.Background(), eventFile)
		if err != nil {
			return err
		}
		if err := event.Start(context.Background()); err != nil {
			log.Error().Err(err).Msg("Error while creating new event... ")
			return err
		}
...
// configuration file has been generated by client that's why it starts event from this function
func (d *daemon) CreateEvent(req *pb.CreateEventRequest, resp pb.Daemon_CreateEventServer) error {
	...
	log.Ctx(resp.Context()).
		...
		Strs("exercises", req.Challenges).
		Msg("create event")
	now := time.Now()

	tags := make([]store.Tag, len(req.Challenges))
	for i, s := range req.Challenges {
		t, err := store.NewTag(s)
		if err != nil {
			return err
		}
		// check challenge before creating event file
		_, tagErr := d.challenges.GetChallengesByTags(t)
		if tagErr != nil {
			return tagErr
		}
		tags[i] = t
	...
	ev, err := d.ehost.CreateEventFromConfig(context.Background(), conf)
	if err != nil {
		return err
	}
	if err := ev.Start(context.Background()); err != nil {
		log.Error().Err(err).Msgf("Error on creating event %s", ev.GetConfig().Name)
	}
...
func (d *daemon) ListChallenges(ctx context.Context, req *pb.Empty) (*pb.ListChallengesResponse, error) {
	var challenges []*pb.ListChallengesResponse_Challenge

	for _, e := range d.challenges.ListChallenges() {
		var tags []string
		for _, t := range e.Tags {
			tags = append(tags, string(t))
		}

		challenges = append(challenges, &pb.ListChallengesResponse_Challenge{
			Name:             e.Name,
			Tags:             tags,
	...
	return &pb.ListChallengesResponse{Challenges: challenges}, nil
}

func (d *daemon) UpdateChallengesFile(ctx context.Context, req *pb.Empty) (*pb.UpdateChallengeFileResponse, error) {
	challenges, err := d.challenges.UpdateChallengesFile(d.conf.ExercisesFile)
	if err != nil {
		return nil, err
	}
	// update event host challenges store
	if err := d.ehost.UpdateEventHostChallengesFile(challenges); err != nil {
		return nil, err
	}
	// update daemons' exercises store
	d.challenges = challenges
	return &pb.UpdateChallengeFileResponse{
		Msg: "Challenges file updated ",
	}, nil

}

// todo : review
func (d *daemon) ResetChallenge(req *pb.ResetChallengeRequest, stream pb.Daemon_ResetChallengeServer) error {
	log.Ctx(stream.Context()).Info().
		Str("evtag", req.EventTag).
		Str("chtag", req.ChallengeTag).
		Msg("reset challenge")
	...
	if req.Teams != nil {
		for _, reqTeam := range req.Teams {
			...
			if err := lab.Environment().ResetByTag(stream.Context(), req.ChallengeTag); err != nil {
				return err
			}
...
func (d *daemon) ListEvents(ctx context.Context, req *pb.ListEventsRequest) (*pb.ListEventsResponse, error) {
	...
	for _, ev := range d.eventPool.GetAllEvents() {
		...
		events = append(events, &pb.ListEventsResponse_Events{
			...
			ChallengeCount: int32(len(conf.Lab.Challenges)),
